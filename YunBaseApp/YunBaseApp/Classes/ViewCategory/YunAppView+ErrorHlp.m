...
@implementation YunAppView (ErrorHlp)

#pragma mark - rqt view

- (void)showRqtErrorView:(
                YunErrorHelper *)error
...
- (void)showError:(YunErrorHelper *)helper superView:(UIView *)view rst:(void (^)(void))rst {
    ...
    if (self.hideStateView) {
        [self showRqtAlert:err.getMsgByMode sv:self result:rst];
    }
    else {
